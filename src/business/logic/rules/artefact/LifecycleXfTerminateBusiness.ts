...
/**
 * `lifecycle-xf-terminate` (G9, structural) — § 8.3.
 *
 * A component of the artifact other than `XF` itself invokes
 * `terminate()` on the `XF` element. Reported on the invoking
 * component.
 */
export class LifecycleXfTerminateBusiness extends RuleBusiness {
  readonly id = 'lifecycle-xf-terminate'
  readonly group = 9
  readonly verifiability: Verifiability = 'structural'
  readonly description = 'A component other than XF itself invokes terminate() on the XF element.'
    ...
    for (const c of artefact.components) {
      ...
      while ((m = re.exec(sf.text)) !== null) {
        ...
        violations.push({
          ...
          message: `${c.name} invokes XF.terminate(). terminate() on the XF element is reserved to XF itself or the external start point.`,
